...
class VolleyballDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return self.fnames[index], self.bboxes[index], torch.from_numpy(buffer), torch.from_numpy(labels), \
    ...
    def load_frames(self, file_dir, bbox_dir):
        ...
        for idx in range(frame_count):
            ...
            #    buffer[i][n+6][:] = person
            #if right_num<6:
            #    buffer[i][right_num+6:] = buffer[i][6+right_num]
            ...
            dist_index = np.argsort(dist[idx], axis=1)
            # for id in range(12):
            #     frame_show = frame
