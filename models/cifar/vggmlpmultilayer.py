...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000, pretrain=False):
        ...
        self.mlp3 = MLP(input_dim=512, hidden_dim=256, output_dim=128)
    ...
    def forward(self, x):
        ...
        node2 = self.mlpMultilayer(base_out, N, T, M)
    ...
    def mlpMultilayer(self, x, N, T, M):
        node0 = torch.zeros(N*T, M, 512).cuda()
        for i in range(0, M, 3):
            node0[:,i,:] = self.mlp1(torch.cat((x[:,i,:], x[:,i+1,:]), dim=1)) + \
            self.mlp1(torch.cat((x[:,i,:], x[:,i+2,:]), dim=1))
            node0[:,i+1,:] = self.mlp1(torch.cat((x[:,i+1,:], x[:,i,:]), dim=1)) + \
            self.mlp1(torch.cat((x[:,i+1,:], x[:,i+2,:]), dim=1))
            node0[:,i+2,:] = self.mlp1(torch.cat((x[:,i+2,:], x[:,i,:]), dim=1)) + \
            self.mlp1(torch.cat((x[:,i+2,:],x[:,i+1,:]), dim=1))


        node1 = torch.zeros(N*T, M, 256).cuda()
        for m in range(0, M, 6):
            for i in range(int(M/2)):
                for j in range(i):
                    node1[:,m+i,:] += self.mlp2(torch.cat((node0[:,m+i,:], node0[:,m+j,:]), \
                    dim=1))
                for j in range(i+1, int(M/2)):
                    node1[:,m+i,:] += self.mlp2(torch.cat((node0[:,m+i,:], node0[:,m+j,:]), \
                    dim=1))

        node2 = torch.zeros(N*T, M, 128).cuda()
        for i in range(M):
            for j in range(i):
                node2[:,i,:] += self.mlp3(torch.cat((node1[:,i,:], node1[:,j,:]), dim=1))
            for j in range(i+1, M):
                node2[:,i,:] += self.mlp3(torch.cat((node1[:,i,:], node1[:,j,:]), dim=1))
