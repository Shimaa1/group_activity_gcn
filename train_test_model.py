...
def train(epoch, cuda, train_data_loader, E_model, E_solver, G1_model, G1_solver, G2_model, G2_solver, D_model,
	...
	for i_batch, sample_batched in enumerate(train_data_loader):
		...
		# labels for GAN
		ones_label = Variable(torch.ones(data_batched.size(1)) + (torch.rand(data_batched.size(1)) - 0.5) * 0.2).cuda()  # full videos
		zeros_label = Variable(torch.zeros(data_batched.size(1)) + torch.rand(data_batched.size(1)) * 0.3).cuda()  # partial videos

		# batch normalization
		max_len = train_data_loader.dataset.max_len
		X_full = data_batched.transpose(0, 2)
		X_full = F.avg_pool1d(X_full, max_len, stride=1)
		X_full = X_full.transpose(0, 2)
		X_full = torch.squeeze(X_full)
...
def test(epoch, cuda, test_data_loader, E_model, G1_model, G2_model, D_model, test_file, num_class):
