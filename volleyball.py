...
from dataset_vol2 import VolleyballDataset
...
def main():
    ...
    trainloader = DataLoader(VolleyballDataset(split='train', transforms=transform_train), batch_size=args.train_batch, shuffle=True, num_workers=args.workers)

    # testset = dataloader(root='./data', train=False, download=False, transform=transform_test)
    testloader = DataLoader(VolleyballDataset(split='test', transforms=transform_test), batch_size=args.test_batch, shuffle=False, num_workers=args.workers)
...
def test(testloader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        if use_cuda:
            # inputs, targets, dists = inputs.cuda(), targets.cuda(), dists.cuda()
